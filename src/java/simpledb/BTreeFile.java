...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid,
		...
		while (iter.hasNext()) {
			...
			if (bTreeEntry.getKey().compare(Predicate.Op.GREATER_THAN, f)) {
				break;
			}
			lastBTreeEntry = bTreeEntry;
		}
		...
		if (f == null) {
			bTreeLeafPage = findLeafPage(tid, dirtypages, lastBTreeEntry.getLeftChild(), perm, f);
		} else if (lastBTreeEntry.getKey().compare(Predicate.Op.GREATER_THAN, f)) {
			bTreeLeafPage = findLeafPage(tid, dirtypages, lastBTreeEntry.getRightChild(), perm, f);
		} else {
			bTreeLeafPage = findLeafPage(tid, dirtypages, lastBTreeEntry.getLeftChild(), perm, f);
		}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page,
		...
		BTreeLeafPage returnPage; // should be the one where given field is inserted.

		BTreePageId rightSiblingId = page.getRightSiblingId(); // need to update this in last

		int numOfTuples = page.getMaxTuples();
		int halfFull = numOfTuples / 2; // what if numOfTuples is odd number ???
		...
		while (iter.hasNext()) {
			...
			if (forFirstTime) {
				for (int i = 0; i < tuple.getTupleDesc().numFields(); i++) {
					if (field.equals(tuple.getField(i))) {
						fieldIndex = i;
						break;
					}
				}
			...
			if (halfFull > 0) {
				if (field.compare(Predicate.Op.LESS_THAN, tuple.getField(fieldIndex))) {
